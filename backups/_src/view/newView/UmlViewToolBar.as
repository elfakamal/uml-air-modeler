package view.newView
{
	...
	public class UmlViewToolBar extends Container
	{
		 ...
		 * 
		 */
		protected var _holder					:ApplicationControlBar		= null;
		...
		public function UmlViewToolBar()
		{
			...
			_holder					= new ApplicationControlBar();
		...
		private function initGradients():void
		{
			...
			leftGradientBoxMatrix.createGradientBox(_leftGradientWidth, height, 0, 0, 0);
			...
			_leftGradientSprite.graphics.drawRect(0, 0, _leftGradientWidth, height);
			
			var rightGradientBoxMatrix:Matrix = new Matrix();
			rightGradientBoxMatrix.createGradientBox(_rightGradientWidth, height, 0, 0, 0);
			...
			_rightGradientSprite.graphics.drawRect(0, 0, _rightGradientWidth, height);
		}
		...
		private function paint():void
		{
			...
			_rightGradientSprite.x		= width - _rightGradientWidth;
		...
		private function onWindowResize(e:FlexNativeWindowBoundsEvent):void
		{
			...
			if (_topLineSprite.width < this.width)
			{
		...
		protected function onScrollingTimer(e:TimerEvent):void
		{
			if (_isMouseOverSide && _mouseOverSide != null)
			{
				...
				{
					if (_holder.x > width - _holder.width - _rightGradientSprite.width)
					{
						_holder.x = _actualX = _holder.x - _scrollingTime * 2;
					}
					else
			...
			if ((_holder.x < _leftGradientSprite.x + _leftGradientSprite.width) ||
				(_holder.x >  width - _holder.width - _rightGradientSprite.width))
			{

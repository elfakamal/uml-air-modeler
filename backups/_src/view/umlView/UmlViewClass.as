package view.umlView
{
	 ...
	 * 
	 */
	public class UmlViewClass extends UmlViewRegularNode
	{
		...
		protected function layoutContextMenu():void
		{
			_umlAddActionContextMenu.x =	x + unscaledWidth + 1 + 
											UmlViewNodeManagmentPicker.PICKER_MARGIN + 
											UmlViewNodeManagmentPicker.PICKER_MAX_WIDTH;
...
//				addEventListener(KeyboardEvent.KEY_DOWN,	keyDownHandler);
		...
		protected function onConstantAdded(e:UmlEvent):void
		{
			...
			if (constant != null)
			{
				constant.addEventListener(UmlEvent.NODE_SELECTED,	onFieldSelected);
				constant.addEventListener(UmlEvent.NODE_DESELECTED,	onFieldDeselected);
			}
		}
		...
		protected function onAttributeAdded(e:UmlEvent):void
		{
			...
			if (attribute != null)
			{
				attribute.addEventListener(UmlEvent.NODE_SELECTED,		onFieldSelected);
				attribute.addEventListener(UmlEvent.NODE_DESELECTED,	onFieldDeselected);
			}
		}
		...
		protected function onOperationAdded(e:UmlEvent):void
		{
			...
			if (operation != null)
			{
				operation.addEventListener(UmlEvent.NODE_SELECTED,		onFieldSelected);
				operation.addEventListener(UmlEvent.NODE_DESELECTED,	onFieldDeselected);
			}
		}

	...
	public class UmlModel extends EventDispatcher 
	{
		...
		model function addType(type:IUmlModelType):void
		{
			...
			else
			{
				trace
				(
					"UmlModel.model::addType(type) : " +
					"type is NULL, see what's going on!!!"
				);
			}
		}
		...
		model function removeTypeByUID(uid:String):void
		{
			...
			else
			{
				trace
				(
					"UmlModel.model::removeTypeByUID(uid) : " +
					"uid is empty, see what's going on!!!"
				);
			}
		}
		...
		model function addChildToElement(parentElement	:IUmlModelElement, 
										 childElement	:IUmlModelElement):void
		{
			if (childElement == null || parentElement == null)
			{
				trace ("le parentElement fourni n'existe pas");
				return;
			}
		...
		{
			var oldUmlNode:IUmlModelElement = null;
			
			if (newUmlNode)
			{
				oldUmlNode = UmlControler.getInstance().getNodeById(elementUID, ownedElements);

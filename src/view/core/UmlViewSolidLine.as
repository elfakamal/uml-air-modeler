package view.core
{
	...
	public class UmlViewSolidLine extends UIComponent
	{
		...
		protected static const _invertedPosition	:Array	= [0, 3, 4, 1, 2];
		...
		protected var _firstEndPosition		:uint					= 0;
		protected var _lastEndPosition		:uint					= 0;
		...
		public function addLineUpdaters(lineUpdaters:Array):void
		{
			for (var i:int = 0; i < lineUpdaters.length; i++)
			{
		...
		protected function onSideMouseDown(event:MouseEvent):void
		{
			...
			if (_movedSide != null && 
				_movedSide.parent != null /* && 
				!_movedSide.parent.hasEventListener(MouseEvent.MOUSE_MOVE) &&
				!_movedSide.parent.hasEventListener(MouseEvent.MOUSE_UP) */ )
			{
				_movedSide.parent.addEventListener(MouseEvent.MOUSE_MOVE,	onSideMouseMove);
				_movedSide.parent.addEventListener(MouseEvent.MOUSE_UP,		onSideMouseUp);
			}
		}
		...
		protected function onSideMouseUp(event:MouseEvent):void
		{
			if (_movedSide != null && 
				...
				_movedSide.parent.hasEventListener(MouseEvent.MOUSE_UP) */)
			{
				_movedSide.parent.removeEventListener(MouseEvent.MOUSE_MOVE,	onSideMouseMove);
				_movedSide.parent.removeEventListener(MouseEvent.MOUSE_UP,		onSideMouseUp);
			}
			...
			if (_sides != null && _sides.length == 2)
			{
					...
					side2 != null && side2.parent != null)
				{
					...
					else
					{
						...
						//render only the specified direction.
						renderLine(direction);
						
						isUpdated = true;
					}
					
					//écouté par UmlViewAssociation
					if (isUpdated)
					{
						dispatchEvent(new UmlViewEvent(UmlViewEvent.LINE_UPDATED));
					}
				}
			}
		}
		
		private function renderLine(renderDirection:int=0):void
		{
			// side 1
			...
			var computeEndPositionFunction	:Function	= new Function();
			...
			if (_movedPointHandler != null || renderDirection != 0)
			{
					...
					renderDirection != 0)
				{
					if (renderDirection < 0)
					{
						...
						pointsToCheck				= [A1, B1, C1, D1];
						computeEndPositionFunction	= updateFirstEndPosition;
					}
					else if (renderDirection > 0)
					{
						initRectangleCorners(_pointHandlers[_points.length - 2], A1, B1, C1, D1, true);
						
						if( movedPointHandlerIndex >= _pointHandlers.length - 1 )
						{
							initRectangleCorners(_pointHandlers[_points.length - 1], A2, B2, C2, D2, true);
						}
						else
						{
						...
						pointsToCheck				= [A2, B2, C2, D2];
						computeEndPositionFunction	= updateLastEndPosition;
					}
				}
			}
			else
				...
				if (_movedSide != null)
				{
					...
					if (movedSideIndex == 0)
					{
						...
						else
						{
							...
							computeEndPositionFunction = function (points:Array, corner:uint):void
							{
								updateFirstEndPosition([A1, B1, C1, D1], corner);
								updateLastEndPosition([A2, B2, C2, D2], corner);
							}
						}
					}
			...
			//BUUGGGG : 
			//TODO: il faut prendre en compte la direction.
			//ces booléens fonctionnent seulement pour le cas A1 ---> B1
			//et donnent des positions inversées pour la dernière extrémité.
			...
			if (isLeftTop)
			{
				...
				computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.TOP_LEFT);
			}
			else if (isRightTop)
			{
				...
				computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.TOP_RIGHT);
			}
			else if (isRightBottom)
			{
				...
				computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.BOTTOM_RIGHT);
			}
			else if (isLeftBottom)
			{
				...
				computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.BOTTOM_LEFT);
			}
			else
			{
				...
				if (isLeft)
				{
					...
					computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.LEFT);
				}
				else if (isTop)
				{
					left1			= A1.x;
					...
					computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.TOP);
				}
				else if (isRight)
				{
					top1			= B1.y;
					...
					computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.RIGHT);
				}
				else if (isBottom)
				{
					left1			= D1.x;
					...
					computeEndPositionFunction(pointsToCheck, UmlViewBorderPosition.BOTTOM);
				}
			}
		}
		...
		protected function updateFirstEndPosition(points:Array, corner:uint):void
		{
			_firstEndPosition = getEndPosition
			(
				points, 
				corner, 
				getFirstLinkDirection()
			);
		}
		
		protected function updateLastEndPosition(points:Array, corner:uint):void
		{
			var position:uint = 0;
			...
			(
				points, 
				corner, 
				getLastLinkDirection()
			);
			
			_lastEndPosition = _invertedPosition[position];
		}
		
		public function getFirstEndPosition():uint
		{
			return _firstEndPosition;
		}
		
		public function getLastEndPosition():uint
		{
			return _lastEndPosition;
		}
		
		protected function getEndPosition(	points		:Array, 
											corner		:uint, 
											direction	:uint):uint
		{
			var position:uint = 0;
			...
			{
				if (corner == UmlViewBorderPosition.TOP		|| 
					corner == UmlViewBorderPosition.RIGHT	|| 
					corner == UmlViewBorderPosition.BOTTOM	|| 
					corner == UmlViewBorderPosition.LEFT)
				{
					return corner;
				}
				...
				switch (true)
				{
					case (corner == UmlViewBorderPosition.TOP_LEFT) :
						
						position = UmlViewBorderPosition.TOP;
						
						if (direction == UmlViewSolidLine.HORIZONTAL)
						{
							position = UmlViewBorderPosition.LEFT;
						}
					
					break;
					case (corner == UmlViewBorderPosition.TOP_RIGHT) :
					
						position = UmlViewBorderPosition.TOP;
						
						if (direction == UmlViewSolidLine.HORIZONTAL)
						{
							position = UmlViewBorderPosition.RIGHT;
						}
						
					break;
					case (corner == UmlViewBorderPosition.BOTTOM_RIGHT) :
						
						position = UmlViewBorderPosition.BOTTOM;
					...
					break;
					case (corner == UmlViewBorderPosition.BOTTOM_LEFT) :
		...
		protected function getFirstLinkDirection():uint
		{
		...
		protected function getLastLinkDirection():uint
		{
			var link		:Sprite	= null;
			var direction	:uint	= 0;
		...
		protected function onPointHandlerMouseDown(e:MouseEvent):void
		{
			...
			if (_movedPointHandlerPoint.x != 0 && _movedPointHandlerPoint.y != 0)
			{
				...
				if (movedPointHandlerIndex == 0 || 
					movedPointHandlerIndex == _pointHandlers.length - 1)
				{
					//écouté par UmlViewAssociation
					dispatchEvent(new UmlViewEvent(UmlViewEvent.LINE_SIDE_POINT_CHANGE_BEGIN));
				}
			}
		...
		protected function onPointHandlerMouseUp(e:MouseEvent):void
		{
			...
			if (_movedPointHandlerPoint.x != 0 && _movedPointHandlerPoint.y != 0)
			{
				...
				{
					//écouté par UmlViewAssociation
					dispatchEvent(new UmlViewEvent(UmlViewEvent.LINE_SIDE_POINT_CHANGE_END));
				}
			}
		...
		protected function updatePointAt(x:Number, y:Number, index:int):void
		{
			...
			updatePointHandlerPosition(newPoint, index);
		...
		protected function removePointHandlerAt(index:int):void
		{
			...
			if (index >= 1 && index < _pointHandlers.length - 1)
			{
				...
				if (pointHandler != null)
				{
					...
					//écouté par UmlViewAssociation
					dispatchEvent(new UmlViewEvent(UmlViewEvent.LINE_POINT_REMOVED));
				}
			}
		}

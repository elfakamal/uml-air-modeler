package view.newView
{
	...
	import view.newView.associations.UmlViewGeneralizationSet;
...
//				dispatchEvent(umlEvent);
		...
		public function verticalMergeGeneralizations(
											superClass		:UmlViewClass, 
											generalizations	:Array):void
		{
			var closerClass		:UmlViewClass			= null;
			var currentClass	:UmlViewClass			= null;
			...
			{
				for (var i:int = 0; i < generalizations.length; i++)
				{
					generalization = generalizations[i];
					
					if (generalization != null && generalization.subClass != null)
					{
						currentClass = generalization.subClass;
		...
		protected function onAssociationNew(event:Event):void
		{
			var Relationship			:Class				= null;
			var relationshipInstance	:UmlViewAssociation	= null;
			var umlEvent				:UmlEvent			= null;
			
			if (event is UmlEvent)
			{
				umlEvent = event as UmlEvent;
				
				UmlControler.getInstance().setMode("association");
				
				Relationship = umlEvent.getAssociationCreationClass();
				
				if (Relationship != null)
				{
					relationshipInstance = new Relationship(this);
					
					_ownedElements.push(relationshipInstance);
					
					relationshipInstance.processCreation((event as UmlEvent).getFirstSide());
				}
			}
		}
		...
		private function onClassAdded(e:UmlEvent):void
		{
			if (e.getAddedElement().owner.uid == uid)
			{
				...
				var elementAddedCommand		:ICommand			= null;
...
//				_ownedElements.push(viewClass);
		...
		{
			var addedElement			:UmlViewElement		= null;
			var elementAddedCommand		:ICommand			= null;
			
			addedElement = event.currentTarget as UmlViewElement;
		...
		protected override function onElementDeleted(e:UmlEvent) : void
		{
			...
			var i						:uint				= 0;
			var removeElementCommand	:ICommand			= null;
			
			for (i = 0; i < _ownedElements.length && !found; i++)
			{

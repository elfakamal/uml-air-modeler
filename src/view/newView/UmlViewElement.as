package view.newView
{
		...
		implements	IUmlViewElement
	{
		...
		{
			var selectedNodesCount:int = 0;
			
			super.keyDownHandler(event);
			
			if (event.keyCode == Keyboard.F2)
			{
				selectedNodesCount += UmlSelectionControler.getInstance().getSelectedRegularNodes().length; 
				selectedNodesCount += UmlSelectionControler.getInstance().getSelectedAssociations().length; 
				selectedNodesCount += UmlSelectionControler.getInstance().getSelectedFeatures().length;
		...
		protected function initListeners():void
		{
			...
			addEventListener(UmlEvent.ELEMENT_SELECTED,		onSelected);
			addEventListener(UmlEvent.ELEMENT_DESELECTED,	onDeselected);
		}
		...
		protected function createContextMenuRuntimeItems():void
		{
			//to override in subclasses.
		}
		...
		/**
		 * if you want to customize this function, do it in subclasses
		 * by overriding it.
		 * 
		 * @param element
		 * @return 
		 * 
		 */
		public function addElement(element:IUmlViewElement):IUmlViewElement
		{
			var addedElement:IUmlViewElement = null;
			
			if (element != null)
			{
				addedElement = this.addChild(element as DisplayObject) as IUmlViewElement;
				
				if (addedElement != null)
				{
					_ownedElements.push(addedElement);
					return addedElement;
				}
			}
		 ...
		 * 
		 */
		public function removeElement(element:IUmlViewElement):IUmlViewElement
		{
			var removedElement	:IUmlViewElement	= null;
			var index			:int				= -1;
			
			if (element != null)
			{
				removedElement = this.removeChild(element as DisplayObject) as IUmlViewElement;
				index = _ownedElements.indexOf(element);
				
				if (removedElement != null && index >= 0)
				{
					_ownedElements.splice(index, 1);
		...
		public function setSelected(value:Boolean):void
		{
			...
			if (_isSelected)
			{
				umlEvent = new UmlEvent(UmlEvent.ELEMENT_SELECTED);
				umlEvent.setSelectedElement(this);
			}
			else
			{
				umlEvent = new UmlEvent(UmlEvent.ELEMENT_DESELECTED);
				umlEvent.setDeselectedElement(this);
			}
